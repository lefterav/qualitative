...
class DecodingProbsAnalyzer(FeatureGenerator):
    ...
    def add_features_tgt(self, simplesentence, parallelsentence):
        ...
        if (tool_id == "t1"):
            ...
            attributes = self.analyze_probs_lucy(attributes)
    ...
    def analyze_probs_joshua(self, attributes):
        ...
        for feature_id in logp_list:
            ...
            produced_atts[att_name] = mean
            
            std = numpy.std(diffs)
            att_name = "%s-std" % feature_id
            produced_atts[att_name] = std
            
            var = numpy.var(diffs)
            att_name = "%s-var" % feature_id
            produced_atts[att_name] = var
            
        attributes.update(produced_atts)
        
        return attributes
    
    def analyze_probs_lucy(self, attributes):
        logp_list = {}
        produced_atts = {}
