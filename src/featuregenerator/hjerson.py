...
#                                    TAGOUTENC='latin1'
    ...
    def get_features_strings(self, target, references):
                
        if self.tokenize:
            target = self.tokenizer.process_string(target)
            references = [self.tokenizer.process_string(reference) for reference in references]
        
        target_tag, target_base = self._tag(target)
        reference_tuples = [(tag,base) for tag,base in self._tag(target)]
        reference_tags = [r[0] for r in reference_tuples]
        reference_bases = [r[1] for r in reference_tuples]
        
        return self.analyze(target, target_base, target_tag, references, reference_bases, reference_tags)
...
class BinaryHjerson(Hjerson):
    def analyze(self, hline, basehline, addhline, refs, baserefs, addrefs):
        features = super(BinaryHjerson, self).get_features(self, hline, basehline, addhline, refs, baserefs, addrefs)
        newfeatures = {}
