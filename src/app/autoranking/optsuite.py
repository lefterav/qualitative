...
#            pipeline_run([annotate.analyze_external_features]) 
        ...
        if n == 0:
            ...
            join_jcml(self.training_sets, self.trainset_filename) 
                    
        if n == 10:
            print "fetch test set"
            shutil.copy(self.testset, self.testset_filename)
                        
        if n == 20:
            print "pairwise training set"
            pairwise_ondisk(self.trainset_filename, self.pairwise_trainset_filename,
        ...
        if n == 50:
            ...
            #parallelsentences = DiffGenerator().add_features_batch(parallelsentences)
            #Parallelsentence2Jcml(parallelsentences).write_to_file(self.pairwise_testset_filename)  
            pass
...
#            ret["AUC_test"] = evaluation.scoring.AUC(cv)
        ...
        if n == 100:
            ...
            self.simple_testset = JcmlReader(self.pairwise_testset_filename).get_dataset()
...
#            self.simple_testset = CompactPairwiseDataset(self.simple_testset) #and this should have no effect
    ...
    def save_state(self, params, rep, n):
        ...
        #    pass
        #if n == 50:
        #    Parallelsentence2Jcml(self.testset).write_to_file(self.pairwise_testset_filename) 
