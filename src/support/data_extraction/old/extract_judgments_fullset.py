...
def parse_sentences(judgments, path, config):
    ...
    for line in judgments:
        ...
        for desiredsystem_name in system_names:
            #get the index of each system
            desiredsystem_id = int(system_names.index(desiredsystem_name))
            system_scores[desiredsystem_name] = (int(fields[int(CSV_MAPPING_SCORE[desiredsystem_id])]))
            
                   
                        
            
        #apply majority voting: when the same systems are compared in more than one comparisons, add them together
        system_names_set = set(system_names)
        if sentence_judgments.has_key(system_names_set):
            sumlist = []
            #sum the two vectorss
            for system_name in system_names_set:
                
            for item in sentence_judgments[system_names_set]:
                sumlist.append (item + system_scores[i])
                i += 1
            sentence_judgments[system_names_set] = sumlist
        else:
            #key empty, just initialize it
            sentence_judgments[system_names_set] = system_scores
