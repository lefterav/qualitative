...
class TaraXUWriter(XmlWriter):
    ...
    def convert_to_xml(self, parallelsentences):
        ...
        jcml = doc_xml.createElement("doc")
        ...
        for ps in parallelsentences:
            ...
            parallelsentence_xml = doc_xml.createElement("sentence")
            ...
            for tgt in ps.get_translations():
                tgt_xml = self.__create_xml_sentence__(doc_xml, tgt, "target")
                parallelsentence_xml.appendChild( tgt_xml )

            #add reference as a child of parallel sentence
            if ps.get_reference():
                ref_xml = self.__create_xml_sentence__(doc_xml, ps.get_reference(), "reference")
