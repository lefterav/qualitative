...
from ml.lib.scikit.scikit import SkRegressor, TerRegressor
...
def train_ter_separately(config, dataset, desired_parallel_attributes, desired_source_attributes, desired_target_attributes):
    regressors = []
    ...
    return TerRegressor(config, regressors)
...
#    dataset = JcmlReader(input_xml_filename)
    ...
    ter_regressor = train_ter_separately(config, dataset, desired_parallel_attributes, desired_source_attributes, desired_target_attributes)
    scores = ter_regressor.cross_validate_start(cv=5)
